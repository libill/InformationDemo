...
public class RainAnimotion extends SurfaceView implements
	...
	private String TAG = "RainAnimotion";
	...
	/** 纵坐标 **/
	private static float[] rainNum = { 6, 6, 6, 12, 12, 12, 18, 18, 18, 12, 18,
			6, 18, 12, 12, 18, 18, 6, 6, 12, 12, 12, 18, 18 };// 24个温度值
	/** 横坐标 **/
...
//	}
	...
	void drawChart(int length) {
		...
		Canvas canvas = sfh.lockCanvas(new Rect(lift, oldX - length, lift
				+ gapX * 24, oldX));// 范围选取正确
		Log.i(TAG, "oldX = " + oldX + "  length = " + length);
		...
		for (int j = 0; j < rainNum.length; j++) {
			...
			if (temMax < 0.8) {
				if (rainNum[j] == 0) {
					canvas.drawRect(new RectF(cx, bottom - 2, dx, bottom),
							mLinePaint);// 当雨量值是0时，绘制2px的矩形，表示这里有值
				} else {
					if ((rainNum[j])/(temMax) > 2/3f){
						mLinePaint.setColor(Color.RED);
					} else if ((rainNum[j])/(temMax) > 1/3f){
						mLinePaint.setColor(Color.BLUE);
					} else {
						mLinePaint.setColor(Color.YELLOW);
					}
					canvas.drawRect(new RectF(cx, dy, dx, bottom),
							mLinePaint);
				}
				...
				if (rainNum[j] == 0) {
					...
					canvas.drawRect(new RectF(cx, cy, dx, bottom),
							mLinePaint);
				}
			}
